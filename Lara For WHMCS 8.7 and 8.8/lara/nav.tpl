          ...
          <ul class="sidebar-menu">
		    ...
		    {if $sidebar eq "home"}<li class="treeview active" id="SideMenu-Home">
				...
				<ul class="treeview-menu">
					...
					{if in_array("Create Invoice",$admin_perms)}<li><a href="{routePath('admin-billing-invoice-new')}" class="open-modal" data-modal-title="{lang key='addNew.invoice'}" {if !empty($anyGatewaysActivated)}data-btn-submit-id="btnCreateInvoice" data-btn-submit-label="{lang key='invoices.create'}"{/if}><i class="fas fa-file-invoice-dollar"></i>{lang key='addNew.invoice'}</a></li>{/if}
